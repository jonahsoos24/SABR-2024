...
s2023 %>%
  mutate(
    ts_fb = as.numeric(if_else(strikes == 2 & description == "foul", 1, 0))
  ) %>%
  group_by(game_pk, at_bat_number) %>%
  mutate(
    pitches_after_ts_fb = -(row_number() - first(row_number()[ts_fb == 1])),
    pafb = if_else(pitches_after_ts_fb < 0, NA, pitches_after_ts_fb),
    DRE = sum(delta_run_exp, na.rm = TRUE)
  ) %>%
  ungroup() -> ts_fb_count

test <- ts_fb_count %>% select(game_pk, at_bat_number, pitch_number, strikes, balls, ts_fb, pitches_after_ts_fb, pafb)
...
write_rds(final_data, "2023wPreds.rds")

mlbraw2 %>% filter(year == 2023) %>%
  group_by(description, events) %>%
  summarise(
    dre_2023 = mean(delta_run_exp, na.rm = TRUE)
  ) -> temp
